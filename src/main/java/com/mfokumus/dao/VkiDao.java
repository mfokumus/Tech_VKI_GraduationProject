...
public class VkiDao implements IDaoGenerics<VkiDto> , Serializable {
    ...
    public String allDelete() {
        try(Connection connection = getInterfaceConnection()){
            ...
            //TRANSACTION YAPISINI YAZALIM
            connection.setAutoCommit(false); // Default:true
            String sql ="DELETE FROM `cars`.`vki`";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            // executeUpdate: create, delete, update için kullanılır.
            int rowsEffected = preparedStatement.executeUpdate();
            // eğer ekleme yapılmamışsa -1 değerini döner
            if (rowsEffected > 0) {
                System.out.println(VkiDao.class+ " Basarili Bütün Silme Tamamdir.");
                connection.commit();
            }else {
                System.err.println(VkiDao.class+ "!!! Basarisiz Bütün Silme Tamamdir");
                connection.rollback();
            }
        }catch (SQLException sql){
            sql.printStackTrace();
    ...
    public VkiDto deleteById(VkiDto vkiDto) {
        try(Connection connection = getInterfaceConnection()){
            ...
            // eğer ekleme yapılmamışsa -1 değerini döner
            if (rowsEffected > 0) {
                System.out.println(VkiDao.class+ " Basarili Silme Tamamdir.");
                connection.commit();
            }else {
                System.err.println(VkiDao.class+ "!!! Basarisiz Silme Tamamdir");
                connection.rollback();
            }
        }catch (SQLException sql){
            sql.printStackTrace();
